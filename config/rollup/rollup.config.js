...
function configure(pkg, env, target) {
  ...
  const pkgPath = pkg.name.split('/').pop();
  const input = `packages/${pkgPath}/src/index.ts`
  const deps = []
  ...
  const plugins = [
    ...
    typescript({
      abortOnError: false,
      tsconfig: `./packages/${pkgPath}/tsconfig.json`,
  ...
  if (isUmd) {
    return {
      ...
      output: {
        format: 'umd',
        file: `packages/${pkgPath}/${isProd ? pkg.umdMin : pkg.umd}`,
        exports: 'named',
  ...
  if (isCommonJs) {
    return {
      ...
      output: [
        {
          file: `packages/${pkgPath}/${pkg.main}`,
          format: 'cjs',
          exports: 'named',
          sourcemap: true,
        },
  ...
  if (isModule) {
    return {
      ...
      output: [
        {
          file: `packages/${pkgPath}/${pkg.module}`,
          format: 'es',
          sourcemap: true,
        }
      ],
